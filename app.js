    ...
    modal.innerHTML = `
      <div class="bg-zinc-900 rounded-lg p-8 max-w-md w-full mx-4">
        ...
        <div class="mb-4">
          ...
          <p class="text-zinc-500 text-sm mt-1">Name for the repository (will be created if it doesn't exist)</p>
        </div>
...
if (window.IS_GITHUB_PAGES) {
  ...
  // Wait for DOM to be ready, then disable server-dependent features
  document.addEventListener('DOMContentLoaded', function() {
    console.log('Disabling server-dependent features...');
    ...
    const disableServerFeatures = () => {
      // Disable create lesson button
      const createBtn = document.querySelector('button[onclick="openCreateModal()"]');
      if (createBtn) {
        createBtn.disabled = true;
        createBtn.title = 'Not available in deployed version';
